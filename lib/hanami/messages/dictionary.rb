module Hanami
  module Validations
    module Messages
      ...
      # @api private
      class Dictionary
        # The library with the validation messages to use when no customized message was 
        # defined for an error
        #
        # @since 0.x.0
        attr_reader :library
        ...
        def default_library
          Hanami::Validations::Messages.library
        end
        ...
        def for(error)
          ...
          message_block.nil? ? 
            library.message_for(error) : message_block.call(error_to_display_on(error))
        end
