...
describe 'Predicates: type?(Date)' do
  before do
    @validator = Class.new do
      include Hanami::Validations

      validates(:name) { type?(Date) }
    end
  end
  ...
  it 'returns failing result for missing data' do
    ...
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Date, nil)
    ]
  end
  ...
  it 'returns failing result for blank data' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Date, '')
    ]
  end
  ...
  it 'returns failing result for string that does not represent a date' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Date, '1')
    ]
  end
  ...
  it 'returns failing result for float' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Date, 3.14)
    ]
  end
  ...
  it 'returns failing result for big decimal' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Date, BigDecimal.new(1))
    ]
  end
