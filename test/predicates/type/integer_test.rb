...
describe 'Predicates: type?(Integer)' do
  ...
  it 'returns failing result for missing data' do
    ...
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Integer, nil)
    ]
  end
  ...
  it 'returns failing result for blank data' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Integer, '')
    ]
  end
  ...
  it 'returns failing result for time' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Integer, now)
    ]
  end
end
