...
describe 'Predicates: type?(Time)' do
  before do
    @validator = Class.new do
      include Hanami::Validations

      validates(:name) { type?(Time) }
    end
  end
  ...
  it 'returns failing result for missing data' do
    ...
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Time, nil)
    ]
  end
  ...
  it 'returns failing result for blank data' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Time, '')
    ]
  end
  ...
  it 'returns failing result for float' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Time, 3.14)
    ]
  end
  ...
  it 'returns failing result for big decimal' do
    ...
    result.wont_be :success?
    result.errors.fetch(:name).must_equal [
      Hanami::Validations::Rules::Error.new(:name, :type?, Time, BigDecimal.new(1))
    ]
  end
